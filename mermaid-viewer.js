 ...
 *  - Resolve team-wide Mermaid config from a Confluence page body
...
(function () {
     ...
     *
     * Resolution order:
     *   1. AMVCENTRAL global space - the single source of truth for all Atlassian
     *      surfaces (Confluence and Jira alike). Only the AMVCENTRAL space owner
     *      (org admin) can create or edit the config page, which guarantees that
     *      security-sensitive settings such as securityLevel cannot be overridden
     *      by an individual space owner. This also ensures uniform rendering
     *      across Confluence spaces and Jira, where no space key exists in the URL.
     *   2. MERMAID_DEFAULTS - silent fallback if no config page exists in
     *      AMVCENTRAL, or if the fetch fails for any reason. No error is surfaced.
     *
     * The result is cached in resolvedConfigCache for the lifetime of the content
     * script process (per-tab). browser.storage is intentionally not used - see
     * AMV-25 comments.
    ...
    async function resolveConfig() {
        ...
        const raw      = await fetchPageConfig(GLOBAL_CONFIG_SPACE_KEY);
    ...
    const iconReady = fetch(browser.runtime.getURL('assets/icon.svg'))
        .then(r => r.text())
        .then(svg => {
